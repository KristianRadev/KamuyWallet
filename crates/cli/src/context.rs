...
impl StewardClient {
    ...
    pub async fn get_policy_change_request(&self, id: &str) -> Result<Option<kamuy_steward::types::PolicyChangeRecord>> {
        ...
        let resp = self.build_request(reqwest::Method::GET, &format!("/api/v1/policy/changes/{}", id))
            .send()
            .await?;
    ...
    /// Approve a policy change request with password verification
    pub async fn approve_policy_change(&self, id: &str, _password: &str) -> Result<()> {
        // Validate ID is valid UUID format
        if !is_valid_uuid(id) {
            return Err(anyhow::anyhow!("Invalid policy change ID format: must be valid UUID"));
        }
        ...
        let resp = self.build_request(
            reqwest::Method::POST,
            &format!("/api/v1/policy/changes/{}/approve", id)
        )
        .send()
        .await?;
    ...
    /// Approve a transaction with password verification (TerminalPassword level)
    pub async fn approve_transaction_with_password(&self, id: &str, _password: &str) -> Result<()> {
        // Validate ID is valid UUID format
        if !is_valid_uuid(id) {
            return Err(anyhow::anyhow!("Invalid transaction ID format: must be valid UUID"));
        }
        ...
        .json(&serde_json::json!({"with_password": true}))
        .send()
        .await?;
